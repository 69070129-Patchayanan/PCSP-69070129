...
```text
โจทย์ข้อนี้ให้เขียนโปรแกรมแสดงฤดูกาล สำหรับเดือนและวันที่ที่กำหนด และฤดูกาลจะเปลี่ยนไปตั้งแต่วันที่ 21 ของเดือนที่ 3 หารลงตัว
input: 
บรรทัดที่1 ต้องการเดือน (จำนวนเต็ม) และบรรทัดที่2 ต้องการวัน (จำนวนเต็ม)
output:
ให้แสดงผลเป็นชื่อ ฤดูกาล มี spring, summer, fall และ winter
constraints:
เขากำหนดไว้ว่าฤดูกาลจะเปลี่ยนตั้งแต่วันที่ 21 ของเดือนที่ 3 หารลงตัว ต้องดูดีดีว่าเขาบอกว่าตั้งแต่วันที่ 21 บางคนอาจจะตั้งไว้แค่ว่าถ้าเป็นวันที่ 21 ฤดูกาลจะเปลี่ยนแต่จริงๆต้อง มากกว่าเท่ากับ 21 ไม่งั้นหลังจากวันที่ 21 จะยังคงเป็นฤดูกาลเดิม
```
...
```text
Step1: ตั้งตัวแปร month รับค่าเดือน และตั้งตัวแปร day รับค่าวัน
Step2: สร้างเงื่อนไขตามที่กำหนดว่าตั้งแต่วันที่ 21 ของเดือนที่หารด้วย 3 ลงตัวจะเปลี่ยนฤดูกาล จึงตั้งเงื่อนไขว่าเดือนที่ 3 ตั้งแต่วันที่ 21 จะเปลี่ยนเป็น spring, เดือนที่ 6 ตั้งแต่วันที่ 21 จะเปลี่ยนเป็น summer, เดือนที่ 9 ตั้งแต่วันที่ 21 จะเปลี่ยนเป็น fall และ เดือนที่ 12 ตั้งแต่วันที่ 21 จะเปลี่ยนเป็น winter
Step3: ส่วนทุกเดือนที่ไม่ใช่ตั้งแต่วันที่ 21 ก็ตั้งไว้ตามที่โจทย์กำหนดโดยใช้ in ใน list เพื่อประหยัดการเขียนเยอะ
```
...
```text
เหมือนกับแผนแรกที่วางไว้ แต่ตอนทำดันลืมว่ามันต้องตั้งแต่วันที่ 21 ตอนเช็ค output เลยไม่ได้ตามต้องการ เลยรีบแก้ และลืมใส่ตั้งแต่ input ว่าต้องเป็นจำนวนเต็ม แต่สุดท้ายก็ทำจนได้ตามต้องการ
```
...
```text
ในกรณีที่ลืมว่าต้องตั้งเงื่อนไขเป็นตั้งแต่วันที่ 21 ไม่ใช่วันที่ 21 เฉยๆ
```
...
```text
ในกรณีที่ลืมอ่านโจทย์ว่าฤดูกาลจะต้องเปลี่ยนไปตั้งแต่วันที่ 21 ของเดือนที่หารด้วย 3 ลงตัว
```
...
```text
เพื่อเช็คว่าถ้าเป็นเดือน 12 หลังจากวันที่ 21 แล้วจะวนกลับมาเป็น winter จริงมั้ย
```
