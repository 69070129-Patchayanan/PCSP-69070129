...
```text
โจทย์ข้อนี้ให้คำนวณค่าธรรมเนียมการส่งพัสดุต้นทาง ไป ปลายทางและน้ำหนัก โดยมีอัตราค่าธรรมเนียมที่เขาให้มา

Input:
บรรทัดแรกนับ ต้นทาง,ปลายทาง บรรทัดเดียวเป็นพิมพ์ใหญ่เท่านั้น บรรทัดสองรับค่าน้ำหนัก

Output:
แสดงราคาค่าธรรมเนียมที่คำนวณเสร็จแล้ว แต่ถ้าเส้นทางไม่มีอยู่ในตาราง ให้แสดงว่า Error

Constraints:
Input ต้องรับในบรรทัดเดียว ต้องใช้ .split ต้องเป็นตัวพิมพ์ใหญ่ และข้อนี้ในโจทย์ไม่ได้บอกว่า Output ต้องออกมาเป็นทศนิยมสองต่ำแหน่งแต่ใน testcase เป็น 2 ต่ำแหน่งหมดจึงต้องใช้  f string
```
...
```text
Step 1: รับ input ad1,ad2 เป็นต้นทางปลายทางใช้ .spilt เพื่ออ่านค่าแยกกันแต่อยู่ในบรรทัดเดียวกัน และรับตัวแปร weight เป็นค่าน้ำหนัก
Step 2: ตั้งเงื่อนไขต้นทางปลายทาง ตามตารางที่โจทย์ให้มา
Step 3: แต่ล้ะเงื่อนไขแสดงผลออกมาโดย ค่าธรรมเนียมเริ่มต้น + ค่าธรรมเนียมน้ำหนัก * น้ำหนัก จากตาราง
Step 4:แสดงผลออกมาใช้ f string ทำให้เป็นทศนิยม 2 ตำแหน่ง
```
...
```text
ทำตามแผนแรกที่วางไว้ การใช้ .spilt รับ Input บรรทัดเดียว และการใช้ f string ทำให้เป็นทศนิยม 2 ตำแหน่ง
```
...
```text
เพื่อดูว่าคิดเฉพาะค่าธรรมเนียมเริ่มต้นถูกต้องมั้ย
```
...
```text
เพื่อเช็คตามที่โจทย์บอกว่าภาษาอังกฤษตัวพิมพ์ใหญ่เท่านั้น
```
