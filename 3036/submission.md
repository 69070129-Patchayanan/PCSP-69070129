...
```text
ข้อนี้ให้รับห้องที่เราอยู่ และหาว่าถ้าจะเดินทางไปถึงห้อง 1 จะต้องพังกำแพงน้อยที่สุดกี่กำแพง
Input:
รับตัวแปร room คือห้องที่เราอยู่
Output:
แสดงผลว่าต้องพังน้อยที่สุดกี่กำแพงเพื่อไปห้อง 1 ถ้าไม่พังเลยก็ตอบ 0
constraints:
ข้อนี้ต้องคิดหาสูตรเอง ซับซ้อนและอาจมึนได้ ต้องนึกภาพให้ออกระหว่างห้องบนห้องล่าง
```
...
```text
Step1: อ่านค่าตัวแปร room คือห้องที่เราอยู่เป็นจำนวนเต็ม
Step2: คิดหาสูตรว่าเราต้องพังกี่กำแพงกันแน่
Step3: ตั้งเงื่อนไขว่าถ้าห้องที่เราอยู่คือห้องที่ 1 ให้ตอบ 0
Step4: นอกนั้นให้คิดตามสูตร
```
...
```text
คล้ายแผนแรก แต่สูตรตอนแรกไม่ครอบคลุม และไม่ได้คิดกรณีห้องบนล่าง เลยลองถามเพื่อนว่ามันควรทำยังไงให้เพื่อนอธิบาย และตอนตั้งเงื่อนไขดันใช้เป็น str เลยทำให้ไม่เข้าเงื่อนไข ตอนส่งเลยทำให้ผิด
```
...
```text
ตรวจดูว่าถ้าอยู่ใกล้จุดเปลี่ยนของชั้นจะเป็นยังไง
```
...
```text
ตรวจดูว่าห้องที่อยู่หลังห้องที่ใกล้จุดเปลี่ยนของชั้นจะเป็นยังไง
```
...
```text
ตรวจดูห้องที่ 3 เพราะเป็นห้องเดียวที่พังแค่กำแพงเดียว
```
...
```text
ให้เขาอธิบายเงื่อนไขของชั้นบนชั้นล่าง
```

คุณยังทำอะไรด้วยตนเอง

```text
ลองเขียนเงื่อนไขใหม่ด้วยตัวเอง
```
