...
```text
โจทย์ให้รับค่าจำนวนเต็มบวก 1 จำนวน (N) จากนั้นหาว่าจำนวนเต็มบวกตั้งแต่ N ถึง 0 มีตัวเลขใดบ้างที่หารด้วย 10 ลงตัว
input:
รับค่าจำนวนเต็มบวก N 
output:
แสดงค่าจำนวนเต็มบวกตั้งแต่ N ถึง 0 ที่หารด้วย 10 ลงตัว (คั่นข้อมูลแต่ละตัวด้วยช่องว่าง)
constraints:
โจทย์ข้อนี้ตรง output เขาให้แสดงผลออกมาเป็นบรรทัดเดียว แต่ใช้ for loop เพราะฉนั้นต้องใช้ end มาช่วย
```
...
```text
Step1:  รับ input เป็นจำนวนเต็มบวก N
Step2: ใช้ for in range ตั้งไว้ว่า start จาก N stop ที่ -1 เพราะจะได้จบที่ 0 และ step -10 ให้รันค่าย้อนกลับ
Step3: จากนั้นให้ print โดยใช้ end= ' '
```
...
```text
คล้ายกับแผนแรกแต่เข้าใจผิดดันไปใช้ step -10 แต่จริงๆต้องตั้งเงื่อนไข ให้ถ้า % 10 แล้วลงตัวก็ให้ print ออกมา
```
...
```text
ในกรณีที่ลืมว่าเขาให้ output ออกมาเป็นบรรทัดเดียว
```
...
```text
ในกรณีที่เข้าใจผิดว่าต้องตั้ง step เป็น -10 
```
